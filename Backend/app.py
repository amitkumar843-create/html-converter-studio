...
def save_uploaded_html_to_temp(html: str, original_filename: Optional[str]) -> str:
    ...
    This avoids Render/Linux failure where converter_pdf.py / converter_pptx.py
    may treat a long raw HTML string as a filesystem path and raise:
    OSError: [Errno 36] File name too long
    """
    safe_stem = _safe_stem(original_filename)
    temp_path = UPLOAD_DIR / f"{safe_stem}.html"

    # Avoid overwriting if multiple conversions happen close together.
    counter = 1
...
def normalize_converter_input(html_or_path: str, original_filename: Optional[str]) -> str:
    """
    Passes an existing HTML file path through unchanged.
    Saves raw HTML content to temp file and returns temp file path.
    """
    value = (html_or_path or "").strip()

    # If caller passed a valid file path, keep old/local behavior.
    try:
        possible_path = Path(value)
        if possible_path.exists() and possible_path.suffix.lower() in {".html", ".htm"}:
            return str(possible_path)
    except OSError:
        # Long raw HTML strings can raise OSError when treated as a path.
        pass

    # Otherwise treat it as raw HTML and save to temp file.
...
async def convert_pptx(req: RequestModel, request: Request):
    try:
        converter_input = normalize_converter_input(req.html, req.filename)
        file_path = await generate_pptx(converter_input, original_filename=req.filename)
        return {
            "status": "success",
            "pptx_file": file_path,
            "output_file_name": Path(file_path).name,
...
async def convert_html_file_to_pptx(request: Request, file: UploadFile = File(...)):
    try:
        html = await read_uploaded_html(file)
        html_file_path = save_uploaded_html_to_temp(html, file.filename)
        file_path = await generate_pptx(html_file_path, original_filename=file.filename)
        return {
            "status": "success",
            "pptx_file": file_path,
            "output_file_name": Path(file_path).name,
